<?php if(isset($block)): ?>
    <div class="blogWrapper">
        <div class="blog">
            <?php if(isset($block['posts']) && is_array($block['posts'])): ?>
                <?php foreach($block['posts'] as $post): ?>
                    <div class="post">
                        ...
                        <div class="bottom">
                            <span class="date"><?=$post->createdAt->format('d.m.Y')?></span>
                            <a href="/blog/<?=htmlspecialchars($post->slug ?? '')?>" title="<?=htmlspecialchars($post->title ?? '')?>">
    ...
    <script type="module" src="<?=FRONT_ASSET_URL?>/js/blog.js?v=0.0.1"></script>
<?php endif;?>
